...
function Intro() {
  return (
    <p>
      Use this tool to determine the change in your beverage&apos;s ABV from the
      addition of liquids post-fermentation, like spirits or backsweeters.
    </p>
  );
}
...
function Notes() {
  return (
    <>
      <p>
        Remember to halt fermentation prior to addition to prevent any sugar
        additions from being fermented out.
      </p>
      <p>
        Note that the volumes don&apos;t ask for a dimension (fluid ounces,
        gallons, etc.). It won&apos;t matter as long as the volume for the
        beverage and the liquid addition are in the same measure. Also note that
...
function Form() {
  ...
  const calculate = useCallback(() => {
    ...
    if (Number.isNaN(bevAbv) || bevAbv < 0.0) {
      errorMessages.push(
        <>Enter a non-negative numeric beverage ABV percentage.</>,
      );
    }
    ...
    if (Number.isNaN(addnAbv) || addnAbv < 0.0) {
      errorMessages.push(
        <>Enter a non-negative numeric addition ABV percentage.</>,
      );
    }
    ...
    if (abvChange > 0.0) {
      message = (
        <>
          This adds{" "}
          <span className="font-bold">{abvChange.toFixed(2)}% ABV</span> to your
          beverage, resulting in a beverage with{" "}
          <span className="font-bold">{resultingAbv.toFixed(2)}% ABV</span>.
        </>
      );
      ...
      message = (
        <>
          This results in no change in ABV to your beer, resulting in a beer
          with{" "}
          <span className="font-bold">{resultingAbv.toFixed(2)} % ABV</span>.
        </>
      );
      ...
      message = (
        <>
          ...
          <span className="font-bold">
            {Math.abs(abvChange).toFixed(2)}% ABV
          </span>{" "}
      ...
      <div className="inline-grid grid-cols-2 gap-4">
        <GridInputGroup
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
            setBeverageVolume(e.currentTarget.value)
          }
        />
        <GridInputGroup
          ...
          dimension="%"
          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
            setBeverageAbv(e.currentTarget.value)
          }
        />
        <GridInputGroup
          ...
          inputType="tel"
          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
            setAdditionVolume(e.currentTarget.value)
          }
        />
        <GridInputGroup
          ...
          dimension="%"
          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
            setAdditionAbv(e.currentTarget.value)
          }
        />
      </div>
